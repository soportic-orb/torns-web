@php
    ...
    $image = $shortcode->image ? RvMedia::getImageUrl($shortcode->image) : Theme::asset()->url('images/home/center-code-mockup.png');
@endphp
    ...
    <div class="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
        <div class="grid items-center gap-10 overflow-hidden rounded-3xl bg-torns-primary lg:grid-cols-[1fr_auto]">
            <div class="p-8 text-white sm:p-12">
                ...
                <p class="mt-4 max-w-xl leading-relaxed text-white/85">{{ $description }}</p>
                <div class="mt-8">
                    {!! Theme::partial('components.store-badges', ['size' => 'sm']) !!}
                </div>
            </div>
            <div class="hidden items-end pr-12 lg:flex">
                <img src="{{ $image }}" alt="{{ __('Center code screen in the Torns app') }}" class="w-52 -mb-16 drop-shadow-2xl" width="480" height="1044" loading="lazy">
            </div>
        </div>
    </div>
</section>
